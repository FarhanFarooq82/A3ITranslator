...
public class InMemorySessionService : ISessionService
{
    ...
    private readonly string _instanceId = Guid.NewGuid().ToString("N")[..8]; // Unique instance identifier
        ...
        ISTTProviderSelector providerSelector)
    {
        ...
        _logger.LogDebug("InMemorySessionService instance created: {InstanceId}", _instanceId);
    ...
    {
        var requestId = Guid.NewGuid().ToString("N")[..8]; // Short unique ID for this service call
        ...
        try
        {
           ...
           var session = new SessionModel
            ...
            _logger.LogInformation("[{RequestId}] Created session {SessionId} with STT providers: [{STTProviders}], TTS providers: [{TTSProviders}]", 
                requestId, sessionId, string.Join(", ", assignedSTTProviders), string.Join(", ", assignedTTSProviders));
    ...
    public async Task<Result<SessionModel>> GetSessionAsync(string sessionId)
    {
        ...
        {          
            ...
            {
                _logger.LogWarning("Session {SessionId} not found in instance {InstanceId}. Available sessions: {Count}", sessionId, _instanceId, _sessions.Count);
        ...
        {
            _logger.LogError(ex, "Error getting session {SessionId} from instance {InstanceId}", sessionId, _instanceId);
    ...
    public async Task<Result> AddMessageToSessionAsync(ConversationMessageModel conversationMessage)
    {
        try
        {
            ...
            conversationMessage.SequenceNumber = session.NextSequenceNumber;
