...
public class STTProviderSelector : ISTTProviderSelector
{
    ...
    public List<string> GetAssignedTTSProvidersForSession(string mainLanguage, string targetLanguage)
    {
        ...
        _logger.LogDebug("Available TTS services: {ServiceCount}", _ttsServices?.Count() ?? 0);

        if (_ttsServices == null)
        {
            _logger.LogWarning("No TTS services registered!");
        ...
        {
            _logger.LogDebug("Checking TTS service: {ServiceName}", service.GetServiceName());
            if (TTSSupportsLanguagePair(service.GetServiceName(), mainLanguage, targetLanguage))
            {
                ...
                supportingProviders.Add((service, priority));
                _logger.LogDebug("TTS service {ServiceName} supports language pair with priority {Priority}", service.GetServiceName(), priority);
            }
            else
            {
                _logger.LogDebug("TTS service {ServiceName} does NOT support language pair", service.GetServiceName());
            }
        }
