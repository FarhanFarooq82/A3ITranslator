...
public class SessionController : ControllerBase
{
    ...
    public async Task<ActionResult<SessionResponse>> CreateSession([FromBody] SessionRequest request)
    {
        try
        {
            _logger.LogInformation("Creating new session for languages {MainLanguage} -> {OtherLanguage}",
                request.MainLanguage, request.OtherLanguage);

            var result = await _sessionService.CreateSessionAsync(request);

            if (!result.IsSuccess)
            {
                _logger.LogError("Failed to create session: {Error}", result.ErrorMessage);
            ...
            _logger.LogInformation("Session created successfully: {SessionId}", session.SessionId);
